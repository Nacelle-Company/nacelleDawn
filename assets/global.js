...
// LUKE's DEV
/*
used in: main-product-settings.liquid
Animate on scroll
...
/* END requestAnimationFrame poly */
...
function initMovingElements() {
	for (var i = 0; i < _positions.length; i++) {
		...
		var el = document.getElementsByClassName(
			"animate-bk-img " + _positions[i].name
		)[0];
...
function updateElements() {
	for (var i = 0; i < _movingElements.length; i++) {
		...
		if (_scrollPercent <= p.start.percent) {
			...
			p.target.x =
				p.start.x * _width +
				((p.diff.x * (_scrollPercent - p.start.percent)) /
					p.diff.percent) *
					_width;
			p.target.y =
				p.start.y * _containerHeight +
				((p.diff.y * (_scrollPercent - p.start.percent)) /
					p.diff.percent) *
					_containerHeight;
		}
